...
using Razor.Core.Engine.Services.BehaviorRecorder;
...
internal sealed class DeleteBehaviorLogsHandler : CommandHandlerBase
{
    ...
    public DeleteBehaviorLogsHandler(
        ICloudConnector cloudConnector,
        ICommandDispatcher dispatcher,
        IBehaviorRecorder behaviorRecorder,
        ILogger<DeleteBehaviorLogsHandler> logger)
    ...
    public override async Task HandleAsync(CloudCommand command, CancellationToken cancellationToken)
    {
        ...
        // Cast to concrete type to access DeleteLogsAsync
        if (_behaviorRecorder is BehaviorRecorder concrete)
        {
            await concrete.DeleteLogsAsync(sessionId, cancellationToken).ConfigureAwait(false);
        }
        else
        {
