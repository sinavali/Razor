...
`Razor.Core.Engine` is the headless executable that hosts the Razor Kernel, manages extensions, and communicates with Razor Cloud via a persistent encrypted WebSocket connection. It is the user‑facing binary that traders and quants deploy on their infrastructure.
...
### 2. Communication (`Razor.Core.Engine.Communication`)

#### CloudConnector

Manages the WebSocket connection to Razor Cloud.

- **Connection:** Attempts primary endpoint (`wss://cloud.Razor.io/engine`) with fallback (`wss://cloud.Razor-fallback.io/engine`).
...
### 3. Core Services (`Razor.Core.Engine.Core`)
...
### 4. Extensions (`Razor.Core.Engine.Extensions`)
...
### 5. Kernel Facade (`Razor.Core.Engine.Kernel`)

`KernelService` bridges Engine commands to `Razor.Core.Kernel` operations.
...
### 6. Command Dispatcher (`Razor.Core.Engine.Management.Commands`)
...
### 7. Task Manager (`Razor.Core.Engine.Management.Tasks`)
...
### 8. Cron & Scheduling (`Razor.Core.Engine.Management.Scheduling`)
...
### 9. BehaviorRecorder (`Razor.Core.Engine.Services.BehaviorRecorder`)
...
### 10. Self‑Update (`Razor.Core.Engine.Services.Update`)
...
| `Razor.Core.Shared` | Memory‑mapped tick lists. |
| `Razor.Core.Kernel` | Core trading logic. |
...
1. Publish to a folder:
   ```bash
   dotnet publish src/Razor.Core.Engine --configuration Release --output ./publish
   ```
...
# With credentials
./Razor.Core.Engine --auth=username,password,apikey

# As a service (Windows)
Razor.Core.Engine.exe --service --auth=username,password,apikey

# As a service (Linux)
./Razor.Core.Engine --service --auth=username,password,apikey
```
...
The Engine has **no configuration file**. All operational parameters are supplied by Razor Cloud after authentication. Local configuration is limited to:

- Credentials (`--auth` or interactive prompt)
- Environment variables:
  - `Razor_PRIMARY_ENDPOINT` – Override primary Cloud endpoint.
  - `Razor_FALLBACK_ENDPOINT` – Override fallback Cloud endpoint.
  - `Razor_AUTH_TOKEN` – Base64‑encoded credentials (alternative to `--auth`).
...
- **Unit tests:** `Razor.Core.Sdk.UnitTests` (200+ tests covering SDK contracts).
- **Integration tests:** `Razor.Core.Sdk.IntegrationTests` (covers file I/O, random, calculators, hooks).
...
*This README is intended for Razor core developers and system administrators. For extension development, see the [Razor.Core.Sdk README](src/Razor.Core.Sdk/README.md).*