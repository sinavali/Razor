// -----------------------------------------------------------------------------
// <copyright file="BinaryTransferManager.cs" company="Razor Platform">
//   Copyright (c) Razor Platform. All rights reserved.
// </copyright>
// -----------------------------------------------------------------------------

namespace Razor.Core.Engine.Communication;
...
internal sealed class BinaryTransferManager : IDisposable
{
    ...
    public void StartIncoming(string transferId, string fileName, long totalSize, string checksum, string contentType)
    {
        ...
        var tempPath = Path.Combine(Path.GetTempPath(), $"Razor_incoming_{transferId}.tmp");
