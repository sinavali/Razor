...
`Razor.Core.Kernel` is the closed‑source heart of the Razor trading engine. It contains all trading logic, including backtesting, live trading, genetic optimisation, brokers, hook infrastructure, telemetry, and messaging. This project is the **core runtime** that executes strategies, evaluates fitness, and manages account state.

This project is **not** part of the public SDK. It is referenced by `Razor.Core.Engine` and is never exposed directly to extension developers.

---

## Key Components

### 1. Backtesting (`Razor.Core.Kernel.Backtesting`)
...
### 2. Brokers (`Razor.Core.Kernel.Brokers`)
...
### 3. Clock System (`Razor.Core.Kernel.Clock`)
...
### 4. Configuration (`Razor.Core.Kernel.Configuration`)
...
### 5. Events (`Razor.Core.Kernel.Events`)
...
### 6. Hook System (`Razor.Core.Kernel.Hooks`)
...
### 7. Indicator Registry (`Razor.Core.Kernel.Indicators`)
...
### 8. Messaging (`Razor.Core.Kernel.Messaging`)
...
### 9. Metrics (`Razor.Core.Kernel.Metrics`)
...
### 10. Neural Networks (`Razor.Core.Kernel.NeuralNetworks`)
...
### 11. Optimisation (`Razor.Core.Kernel.Optimization`)
...
### 12. Reporting (`Razor.Core.Kernel.Reporting`)

**Note:** The engine does **not** generate formatted reports. The `ReportGenerator` class exists solely to invoke `report.before_generate` and `report.after_generate` hooks. Report rendering is the responsibility of Razor Cloud.

---

### 13. Telemetry (`Razor.Core.Kernel.Telemetry`)
...
| Dependency | Purpose |
|------------|---------|
| `Razor.Core.Sdk` | Provides all public contracts, domain types, and helpers. |
| `Razor.Core.Shared` | Provides memory‑mapped tick lists and binary file mapping. |
...
- `Razor.Core.Engine` (via `KernelService` facade)
- `Razor.Core.Kernel.UnitTests` and `Razor.Core.Kernel.IntegrationTests`

It has **no public API** – all types are internal to the `Razor.Core` namespace, except those explicitly exposed via `InternalsVisibleTo`.
...
*This README is intended for Razor core developers. For extension development, see the [Razor.Core.Sdk README](src/Razor.Core.Sdk/README.md).*