...
public interface IExecutionProvider
{
    ...
    Task<Chronos.Abstractions.Shared.AdapterOrderResponse> ExecuteOrderAsync(AdapterOrderRequest request);
    /// <summary>Modifies an existing order's SL, TP, or limit/stop price.</summary>
    Task<Chronos.Abstractions.Shared.AdapterOrderResponse> ModifyOrderAsync(long ticket, double? sl = null, double? tp = null, double? price = null);
    /// <summary>Closes a position (or partially closes it).</summary>
    Task<Chronos.Abstractions.Shared.AdapterOrderResponse> ClosePositionAsync(long ticket, double? volume = null);
    /// <summary>Cancels a pending order.</summary>
    Task<Chronos.Abstractions.Shared.AdapterOrderResponse> CancelAsync(long ticket);
    ...
    Task<IReadOnlyList<Chronos.Abstractions.Shared.Position>> GetActivePositionsAsync();
    /// <summary>Returns all currently pending orders.</summary>
    Task<IReadOnlyList<Chronos.Abstractions.Shared.Order>> GetPendingOrdersAsync();
    /// <summary>Fetches symbol properties from the exchange.</summary>
    Task<Chronos.Abstractions.Shared.SymbolProperties?> GetSymbolPropertiesAsync(string symbol, CancellationToken cancellationToken = default);
