...
|-------|------|---------|-------------|
| `AssetClass` | `AssetClass` | `CryptoSpot` | Market category (Forex, CryptoSpot, Equity, …). |
| `MarginMode` | `MarginMode` | `Cross` | Cross or Isolated margin. |
| `PendingTrigger` | `PendingOrderTriggerMode` | `UseAskForBuy` | Which price triggers pending buy orders. |
| `MarginCurrency` | `string` | `"USDT"` | Currency for margin calculations. |
| `ContractSize` | `double` | `1.0` | Size of one standard contract. |
| `TickSize` | `double` | `0.01` | Minimum price increment. |
| `TickValue` | `double` | `1.0` | Monetary value of one tick. |
| `MinVolume` | `double` | `0.0001` | Minimum order volume. |
| `MaxLeverage` | `double` | `1.0` | Maximum allowed leverage for this symbol. |
| `SwapLong` | `double` | `0` | Daily swap rate for long positions (percentage or absolute). |
| `SwapShort` | `double` | `0` | Daily swap rate for short positions. |
| `SwapRolloverHourUtc` | `int` | `21` | UTC hour at which swap is charged. |
| `TripleSwapDayMultiplier` | `double` | `3.0` | Multiplier for triple‑swap days (typically Wednesdays). |
| `FundingRate` | `double` | `0` | Perpetual contract funding rate (per period). |
| `InitialMarginRate` | `double` | `1.0` | Fraction of position value required as initial margin. |
| `MaintenanceMarginRate` | `double` | `0.5` | Fraction below which liquidation may occur. |
| `MakerFeeRate` | `double` | `0.001` | Fee for maker orders. |
| `TakerFeeRate` | `double` | `0.0015` | Fee for taker orders. |
