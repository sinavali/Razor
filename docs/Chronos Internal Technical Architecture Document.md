...
| *(future)* `Chronos.Engine` | User‑facing executable | Private (closed‑source) | Bootstraps the engine, connects to Cloud, manages plugins and lifecycle. |
...
The `Hosts.Console` project has been removed. The future `Chronos.Engine` executable will be the production host.
...
4. **Main loop:**
   ...
   - For each event:
     ...
     - Calls `strategy.OnTick(tick)` (the new synchronous path for determinism).
...
- Other plugin types use their own discovery attributes (`[StrategyName]`, `[RiskManagerName]`, etc.).

### 9.2 Loading Process

1. Scan a configured plugin directory for `.dll` files.
2. For each assembly, load in a new `PluginLoadContext` (unloadable later).
3. The `PluginLoadContext` ensures `Chronos.Core.Abstractions` is loaded from the default context (type sharing), while all other dependencies are resolved from the plugin’s directory.
4. Call `PluginValidator.ValidateAssembly()` to check the SDK version. Reject if the major version is newer than the engine’s SDK major version, unless an explicit compatibility mode is enabled (future). Older major versions are accepted if they pass interface validation.
5. Discover types with discovery attributes or `IStrategy` implementations.
6. Validate that the type implements the required interfaces and that no missing members exist.
7. Instantiate the component and register it.
...
All metrics are registered in a `Meter` named `"Chronos.Metrics"`. The connection state gauge uses a stable metric name (not per‑instance) with tag differentiation.
