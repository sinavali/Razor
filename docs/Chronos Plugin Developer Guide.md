...
**Version:** 1.0.0 LTS  
**Audience:** Plugin developers (strategies, adapters, indicators, risk managers, etc.)  
...
This guide teaches you how to create plugins for Chronos. A plugin is a .NET DLL that implements one or more public interfaces from the `Chronos.Core.Abstractions` NuGet package. The engine discovers and loads plugins at runtime through isolated contexts.

In v1.0.0 LTS, the following plugin types are supported:

- **Adapter** – Connects Chronos to a broker or exchange.
- **Strategy** – Trading logic that reacts to ticks and places orders.
- **Indicator** – Technical analysis tools (SMA, RSI, etc.).
- **Risk Manager** – Portfolio‑level risk gate that can block orders.
- **Position Sizer** – Calculates how much volume to trade based on account state and risk parameters.
- **Market Regime Detector** – Classifies the current market (trending, range‑bound, volatile).
- **Execution Algorithm** – Smart order execution (TWAP, VWAP, Iceberg).
- **Fitness Model** – Scores a backtest result (higher = better).
- **Simulation Friction** – Applies slippage and commission in backtesting.
- **Notification Channel** – Sends alerts (email, Telegram, etc.).
- **Metrics Provider** – Calculates custom performance metrics.
- **Report Generator** – Produces formatted reports (HTML, PDF, Excel).
- **Market Data Provider** – Provides historical and live data independently of execution.

All plugin types share the same versioning, discovery, and deployment mechanisms.
...
A plugin is a .NET assembly (DLL) that implements one or more public interfaces from the SDK. Plugins never reference the engine directly; they only depend on `Chronos.Core.Abstractions`.
...
The SDK includes helper types like `ChronosRandom` (portable RNG), `TickWindow`, `BinaryDataMapper`, etc. These are helpers for your plugin code; you are free to use them or implement your own. However, any randomness that affects trading decisions must derive from the master seed (see §8.4).
...
When a backtest starts, the engine may be configured with a warm‑up window count (`WarmupWindowCount`). During warm‑up:

- Ticks are still processed and indicators receive data, so they can prime their buffers.
- **No orders are accepted.** `IBroker.IsWarmup` returns `true`. Any call to `ExecuteMarketOrderAsync`, `PlacePendingOrderAsync`, etc. will return a response with `Success = false` and `ErrorMessage = "Orders not allowed during warmup."`.
- The warm‑up period ends automatically once the specified number of timeframe windows have completed. After that, `IsWarmup` becomes `false` and trading begins.

Strategies should check `Broker.IsWarmup` if they need to know whether they are still in warm‑up. Usually, you don’t need to – simply place orders normally; they will be silently rejected during warm‑up.

---

## 7. Developing an Indicator

Indicators derive from the abstract `Indicator` class. The engine manages their lifecycle and provides them with a `TickWindow` (via `IWindowAwareIndicator`) and an `IIndicatorRegistry` (via `IRegistryAwareIndicator`).

```csharp
public class SmaIndicator : Indicator, IWindowAwareIndicator
{
    private int _period;
    ...
    {
        // Use the tick window to get the last 'period' ticks and compute SMA
        // Window is available via base.Window property
    }
}
```

Indicators are created via the registry:
```csharp
var sma = indicators.Get<SmaIndicator>(period);
```

The base class provides a circular buffer and indexer. Override `Calculate` to update your value on each tick.

---

## 8. Developing a Risk Manager

A risk manager implements `IRiskManager`. It is called before every order submission and on every tick.

```csharp
[RiskManagerName("MyRiskManager")]
public class MyRiskManager : IRiskManager
{
    public bool AllowOrder(RiskContext context, out string? rejectReason)
    {
        if (context.FreeMargin < context.PendingOrder?.Volume * 100)
        {
    ...
    {
        return context.MaxDrawdown > 20 ? RiskSignal.Halt : RiskSignal.Allow;
    }
}
```

The discovery attribute is `[RiskManagerName]`.

---

## 9. Developing a Position Sizer

Implements `IPositionSizer`. Computes the volume to trade based on account equity, risk percent, stop distance, and symbol properties.

```csharp
[PositionSizerName("FixedFractional")]
public class FixedFractionalSizer : IPositionSizer
{
    public double CalculateVolume(PositionSizingContext context)
    {
        double riskCapital = context.AccountEquity * context.RiskPercent / 100.0;
        return riskCapital / context.StopLossDistance;
    }
}
```

Discovery attribute: `[PositionSizerName]`.

---

## 10. Developing a Market Regime Detector

```csharp
[MarketRegimeDetectorName("ATRRegime")]
public class AtrRegimeDetector : IMarketRegimeDetector
{
    public MarketRegime Detect(string symbol, TickWindow tickWindow)
    {
...
Discovery attribute: `[MarketRegimeDetectorName]`.

---

## 11. Developing an Execution Algorithm

```csharp
[ExecutionAlgoName("TWAP")]
public class TwapAlgorithm : IExecutionAlgorithm
{
    public async IAsyncEnumerable<ExecutionSlice> GenerateSlicesAsync(
        ExecutionAlgorithmRequest request,
        [EnumeratorCancellation] CancellationToken ct)
    {
        // slice order over time
        yield return new ExecutionSlice
        {
            ScheduledTime = DateTime.UtcNow.AddSeconds(5),
            Volume = request.TotalVolume / 10,
            LimitPrice = null
        };
    }
}
...
Discovery attribute: `[ExecutionAlgoName]`.

---

## 12. Developing a Fitness Model

```csharp
[FitnessModelName("NetProfitMinusDrawdown")]
public class NetProfitFitness : IFitnessModel
{
    public double Evaluate(double finalBalance, double initialBalance,
        double maxDrawdown, double maxDailyDrawdown, int totalTrades,
        IReadOnlyList<Position> history)
    {
        return (finalBalance - initialBalance) - 2 * maxDrawdown;
    }
}
```

Discovery attribute: `[FitnessModelName]`.

---

## 13. Developing a Simulation Friction Model

```csharp
public class ZeroFriction : ISimulationFriction
{
    public double CalculateSlippage(string symbol, OrderType type, double volume, double currentPrice) => 0;
    public double CalculateCommission(string symbol, double volume) => 0;
}
```

Currently, friction models are passed via `StrategySpecification.FrictionModel`. A discovery attribute may be added in future.

---

## 14. Developing a Notification Channel

```csharp
[NotificationChannelName("Telegram")]
...
Discovery attribute: `[NotificationChannelName]`.

---

## 15. Developing a Metrics Provider

```csharp
[MetricsProviderName("CustomMetrics")]
public class CustomMetricsProvider : IMetricsProvider
{
    public IReadOnlyDictionary<string, double> Calculate(ReportRequest request)
    {
        // return extra metrics
        return new Dictionary<string, double>
        {
            ["my_metric"] = request.FinalBalance / request.InitialBalance
        };
    }
}
```

Discovery attribute: `[MetricsProviderName]`.

---

## 16. Developing a Report Generator

```csharp
[ReportGeneratorName("PDF")]
public class PdfReportGenerator : IReportGenerator
{
    public string Format => "PDF";
    public async Task<byte[]> GenerateAsync(ReportRequest request, CancellationToken ct)
    {
        // produce PDF bytes
...
Discovery attribute: `[ReportGeneratorName]`.

---

## 17. Developing a Market Data Provider

```csharp
[MarketDataProviderName("PremiumData")]
public class PremiumDataProvider : IMarketDataProvider
{
    public string ProviderName => "PremiumData";
    // implement IHistoricalDataProvider + ILiveDataProvider
}
```

Discovery attribute: `[MarketDataProviderName]`.

---

## 18. Configuration and Symbol Properties

### 18.1 Strategy Specification
...
### 20.2 Thread Safety for Adapters
...
### 20.3 Determinism for Strategies
...
### 20.4 Avoiding Hard‑coded Market Logic
...
### 22.2 Uploading to Chronos Cloud
